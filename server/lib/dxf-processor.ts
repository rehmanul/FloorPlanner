...
export class DXFProcessor {
  async processDXF(fileContent: string): Promise<ProcessedFloorPlan> {
    try {
      const dxfData = this.parseSync(fileContent);
  ...
  private parseSync(content: string) {
  ...
  private analyzeSpaceUsage(walls: Wall[], doors: Door[], windows: Window[], bounds: Bounds): SpaceAnalysis {
    const totalArea = (bounds.maxX - bounds.minX) * (bounds.maxY - bounds.minY) / 10000;
    ...
    walls.forEach(wall => {
      ...
      wallArea += (length * wall.thickness) / 10000;
    });
